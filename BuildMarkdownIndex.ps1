...
$OutputFile = Join-Path -Path $RootDir -ChildPath "_sidebar.md"

# Create or overwrite the output file
Set-Content -Path $OutputFile -Value ""

# Get all .md files recursively, excluding those in the root folder and not folder tutoriel
$MarkdownFiles = Get-ChildItem -Path $RootDir -Recurse -File -Filter "*.md" | Where-Object {
    $_.DirectoryName -ne $RootDir } | Where-Object {
    -not ($_.FullName -match "\\tutoriel\\")
}
...
# Get all .md files in tutoriel recursively, excluding those in the root folder
$MarkdownFiles = Get-ChildItem -Path $RootDir -Recurse -File -Filter "*.md" | Where-Object {
    $_.DirectoryName -ne $RootDir } | Where-Object {
    ($_.FullName -match "\\tutoriel\\")
}
